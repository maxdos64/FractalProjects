...
class Circle(Shape):
    ...
    def render(self, canvas, mark=False):
        ...
        pygame.draw.circle(canvas, self.fill_color, ogl_point_to_pixel(self.center), ogl_mag_to_length(self.radius))
...
if __name__ == '__main__':
    ...
    # canvas.pack()
    window = pygame.display.set_mode((PIXEL_WIDTH, PIXEL_HEIGHT), FULLSCREEN | DOUBLEBUF)
    fps_clock = pygame.time.Clock()
    
    fractal = FuzzyFractal()

    active = True
    window.fill((0xff, 0xe7, 0xa1))
    while active:
        for event in pygame.event.get():
          if event.type == pygame.QUIT:
             active = False

        fractal.render_frame(window)
